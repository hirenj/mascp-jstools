...
(function(clazz) {
    ...
    const PAN_TWEEN_SYMBOL = Symbol('PAN_TWEEN');
    ...
    clazz.prototype.panTo = function(end,callback) {
        ...
        renderer[PAN_TWEEN_SYMBOL] = tween;
        return new Promise( resolve => {
            tween.to({ pos: end },PAN_TIMING)
            .easing(Easing.Quadratic.InOut)
            .on('update', ({pos}) => {
                if (renderer[PAN_TWEEN_SYMBOL] !== tween) {
                    if ( ! tween.stopped ) {
                        tween.stopped = true;
                        if (callback) {
            ...
            .on('complete', (newstate) => {
                ...
                if (callback) {
                    callback.call(null,still_active_tween);
                }
                resolve(still_active_tween);
            })
            .start();
        });
    };

    clazz.prototype.shouldPanInsteadForZoom = function(zoom) {
        let renderer = this;
        let curr = renderer.zoom;
        return (Math.abs(zoom - curr) < 0.1);
    }
...
(function() {
    ...
    CondensedSequenceRenderer.prototype.importIcons = function(namespace,doc,alt_url) {
        ...
        this._container_canvas.appendChild(new_owner.createElementNS(svgns,'defs'));
...
CondensedSequenceRenderer.prototype.enableScaling = function() {
    bean.add(this,'readerRegistered',function(reader) {
        console.log('Enabling scaling for Sequence alignment');
        let old_result = reader.gotResult;
        let renderer = this;
        reader.gotResult = function() {
            let wanted_id = reader.acc || reader.agi || "";

            let old_get_aas = CondensedSequenceRenderer.prototype.getAminoAcidsByPosition;
            let old_get_pep = CondensedSequenceRenderer.prototype.getAminoAcidsByPeptide;
            let old_sequence = renderer.sequence;
